...
def get_batch_data():
	...
	while True:
		...
		for ind in train_vector:
            ...
            scaler = StandardScaler()  ####For Normalization of the data
            scaler.fit(data)
            X_sc_train = scaler.transform(data)
            NCOMPONENTS = 3
            pca = PCA(n_components=NCOMPONENTS)
            X_pca_train = pca.fit_transform(X_sc_train)
            data=np.reshape(X_pca_train,(data_shape[0],data_shape[1],NCOMPONENTS))
...
def get_eval_data():
	...
	while ind< len(test_vector):
        ...
        scaler.fit(data)
        X_sc_test = scaler.transform(data)
        NCOMPONENTS = 3
        pca = PCA(n_components=NCOMPONENTS)
        X_pca_test = pca.fit_transform(X_sc_train)
        data=np.reshape(X_pca_test,(data_shape[0],data_shape[1],NCOMPONENTS))
...
filepath = "/scratch/cse/mtech/mcs172873/weights/resnet_imbalance/bf/weights-improvement-{epoch:02d}.hdf5"
...
np.savetxt('/home/cse/mtech/mcs172873/LANDSAT/Resnetweights/BF/Test_Results_BF.csv',arr)